...
function listPaddockManagersByName() {
  var names = paddockManagers.map((paddockManager) => ({ "taxNumber":paddockManager.taxNumber, "name":paddockManager.name }));
...
function sortPaddockTypeByTotalArea() {	
	var sumPaddock = paddocks.reduce
	(
		(paddockTypeId, area) => 
		{
			if (!paddockTypeId[area.paddockTypeId]) 
				paddockTypeId[area.paddockTypeId] = { ...area, count: 1 };
			else
			{
				paddockTypeId[area.paddockTypeId].area += area.area;
				paddockTypeId[area.paddockTypeId].count += 1;		
			}

			return paddockTypeId;
		}, {}	
	);
	...
	var paddocksTotal = Object.keys(sumPaddock).map(function(i){
		...
		paddockTypeName = paddockType.find(x => x.id === e.paddockTypeId).name;
		
		return ({ "paddockTypeId": e.paddockTypeId, "paddockTypeName" : paddockTypeName, "area": e.area / convertHectareas });
	});	
		
	paddocksTotal.sort((a,b) => (b.area > a.area) ? 1 : ((a.area > b.area) ? -1 : 0))
...
function sortFarmManagerByAdminArea() {	
	...
	var paddocksTotal = Object.keys(sumPaddock).map(function(i){
		...
		return ({ "paddockManagerId": e.paddockManagerId, "paddockManagerName" : paddockManagerName, "area": e.area / convertHectareas });
	});		
...
function farmManagerNames() {
  var farmNames = [];
  var paddockManager = null;
  
  paddocks.forEach(function(e){
	paddockManager = paddockManagers.find(x => x.id === e.paddockManagerId);
	
	farmNames.push({ "farmId":e.farmId, "paddockManagerId":e.paddockManagerId,"paddockManagerTaxNumber":paddockManager.taxNumber });
  });
  
  var farmResults = [];
  
  farms.forEach(function(e){
	  var paddockFilter = farmNames.filter(x => x.farmId === e.id);
	  	  
	  var ruts = paddockFilter.map((x) => x.paddockManagerTaxNumber).filter(unique).sort();
	  
	  farmResults.push({"farmId": e.id,"farmName":e.name, "ruts": ruts  });
  });  
...
function biggestAvocadoFarms() {
	//Filtro paltos
	var idPaltos = paddockType.find(x => x.name === "PALTOS").id;
	
	var paddocksPaltos = paddocks.filter(x => x.paddockTypeId === idPaltos);	
  
	//Sumo campos
	var sumPaddock = paddocksPaltos.reduce
	(
		(farmId, area) => 
		{
	...
	var paddocksTotal = Object.keys(sumPaddock).map(function(i){
		...
		farmName = farms.find(x => x.id === e.farmId).name;
		
		return ({ "farmId": e.farmId, "farmName" : farmName, "area": e.area });			
	});
	
	//Filtro mayor a 2 hectáreas
	paddocksTotal = paddocksTotal.filter(x => x.area > 2 * convertHectareas);	
	
	//Ordeno decrecientemente las areas
	paddocksTotal.sort((a,b) => (b.area > a.area) ? 1 : ((a.area > b.area) ? -1 : 0))  
...
function farmManagerPaddocks() {
  ...
  paddocks.forEach(function(e){
	farm = farms.find(x => x.id === e.farmId);
	
	paddocksManager.push({ "paddockManagerId":e.paddockManagerId, "farmId":e.farmId, "farmName":farm.name });
  });
  
  var paddockManagersResults = [];
  
  paddockManagers.forEach(function(e){
	  var paddockFilter = paddocksManager.filter(x => x.paddockManagerId === e.id);
	  
	  var farms = paddockFilter.map((x) => x.farmName).filter(unique).sort();
	  
	  paddockManagersResults.push({"paddockManagerId": e.id,"paddockManagerName":e.name, "farms": farms  });
  });  
  
  return paddockManagersResults;
}
...
function paddocksManagers() {
  ...
  paddocks.forEach(function(e){
	...
	paddocksManager.push({ "key":paddockTypeName + "-" + e.harvestYear, "value": { "id": e.paddockManagerId, "name": paddockManagerName } });
  });  
  
  paddocksManager.sort((a,b) => (a.key > b.key) ? 1 : ((b.key > a.key) ? -1 : 0));
  
  return paddocksManager;
}
...
function newManagerRanking() {
	var newManager = { id: 7, taxNumber: '44136285', name: 'ANDREI SERGIO RODRIGUEZ BARRIGA'};
	
	paddockManagers.push(newManager);
	
	paddocks.push({ paddockManagerId: 7, farmId: 1, paddockTypeId: 4, harvestYear: 2017, area: 900 });
	
	var ranking = sortFarmManagerByAdminArea();
	var count=1;
	ranking.forEach(function(e,index){
		if(e.paddockManagerId===newManager.id) count = index+1;
	});
